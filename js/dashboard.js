...
const CJDashboard = {
  ...
  renderAuditCategories() {
    ...
    const uniqueOutlets = new Set(stores.map(s => s.storeCode || s.id)).size;

    const audits = CJStorage.getAuditsForCurrentGSBH();
    const auditedStoreIds = new Set(audits.map(a => a.storeId));
    const auditedStoresCount = stores.filter(s => auditedStoreIds.has(s.id) || s.isAudited).length;
    ...
    this.setElText("dashOverallCompletedTasks", `${auditedStoresCount} / ${totalStores} Tủ (${uniqueOutlets} KH)`);
    this.setElText("dashOverallTotalTasks", `${totalStores} Tủ đông`);
    ...
    this.setElText("dashSummaryTotalStores", `${totalStores} Tủ (${uniqueOutlets} KH)`);
    this.setElText("dashSummaryCompletedTotal", `${auditedStoresCount} Tủ (${overallRate}%)`);
    this.setElText("dashSummaryDangerTotal", `${dangerStoresCount} Tủ (${dangerStoresCount > 0 ? 'Cần xử lý' : 'An toàn'})`);
    ...
    if (subAssigned) {
      ...
      subAssigned.textContent = `${areaText} • ${totalStores} Tủ đông (${uniqueOutlets} KH)`;
    }

    // Update Quick KPI Summary (Mobile Mode)
    this.setElText("dashMobileSummaryPrograms", "Kiểm tra tủ đông");
    this.setElText("dashMobileSummaryStores", `${totalStores} Tủ (${uniqueOutlets} KH)`);
    this.setElText("dashMobileSummaryCompleted", `${auditedStoresCount} Tủ (${overallRate}%)`);
    ...
    const categories = [
      {
        ...
        barColor: "bg-red-600",
        stats: [
          { label: "Tổng cần kiểm tra", value: `${totalStores} Tủ (${uniqueOutlets} KH)`, color: "text-gray-900 bg-gray-50 border-gray-200 font-bold" },
          { label: "Đã hoàn thành", value: `${auditedStoresCount} Tủ`, color: "text-emerald-800 bg-emerald-50 border-emerald-200 font-bold" },
          { label: "Chưa kiểm tra", value: `${Math.max(0, totalStores - auditedStoresCount)} Tủ`, color: "text-amber-800 bg-amber-50 border-amber-200 font-bold" },
        ...
        actionText: `🛵 Bấm Vào Để Mở Danh Sách Kiểm Tra Tủ Đông (${totalStores} Tủ - ${uniqueOutlets} KH) ➔`
      }
    ];
